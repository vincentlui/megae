...
from mrl.himega.curiosity import HimegaDensityAchievedGoalCuriosity
from mrl.himega.online_her_buffer import HimegaOnlineHERBuffer
...
def main(args):
  ...
  if not args.no_ag_kde:
    if args.use_cg:
      config.ag_kde = RawKernelDensity('cg', optimize_every=1, samples=10000, kernel=args.kde_kernel, bandwidth = args.bandwidth, log_entropy=True)
    else:
      config.ag_kde = RawKernelDensity('ag', optimize_every=1, samples=10000, kernel=args.kde_kernel,
                                       bandwidth=args.bandwidth, log_entropy=True)
  if args.ag_curiosity is not None:
    ...
    if args.ag_curiosity == 'minq':
      ...
      config.ag_curiosity = DensityAchievedGoalCuriosity('ag_kde', max_steps = args.env_max_step, num_sampled_ags=args.num_sampled_ags, use_qcutoff=use_qcutoff, keep_dg_percent=args.keep_dg_percent)
    elif args.ag_curiosity == 'cgkde':
      config.ag_curiosity = HimegaDensityAchievedGoalCuriosity('cg_kde', max_steps = args.env_max_step, num_sampled_ags=args.num_sampled_ags, use_qcutoff=use_qcutoff, keep_dg_percent=args.keep_dg_percent)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--use_cg', action='store_true', help="context goal")
