...
class HimegaOnlineHERBuffer(OnlineHERBuffer):
  ...
  def _setup(self):
    ...
    if self.goal_space is not None:
      items += [("previous_ag", self.goal_space.shape), # for reward shaping
                ...
                ("cg", (2*self.goal_space.shape[0],))]  # concat goal
  ...
  def _process_experience(self, exp):
    ...
    if self.goal_space:
      ...
      if hasattr(self, 'ag_curiosity'):
        if self.ag_curiosity.current_goals is not None:
          behavioral = self.ag_curiosity.current_goals
        if self.ag_curiosity.initial_goals is not None:
          cg = self.ag_curiosity.append_initial_goal(achieved)
  ...
  def sample(self, batch_size, to_torch=True):
    ...
    if self.goal_space:
      ...
      if has_config_her:
        ...
        states, actions, rewards, next_states, dones, _ , _, _, _, goals =\
