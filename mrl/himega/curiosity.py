...
class AchievedGoalCuriosity(mrl.Module):
  ...
  def _setup(self):
    ...
    self.initial_goals = self.env.state['achieved_goal']
  ...
  def _manage_resets_and_success_behaviors(self, experience, close):
    ...
    for i, over in enumerate(experience.trajectory_over):
      if over:  # if over update it
        ...
        self.initial_goals[i] = experience.reset_state['achieved_goal'][i]
  ...
  def _process_experience(self, experience):
    ...
    if np.any(experience.trajectory_over) and len(ag_buffer):
      ...
      sampled_igs = np.tile(experience.reset_state['achieved_goal'][:, None, :], (1, self.num_sampled_ags, 1))
      sampled_cgs = np.concatenate([sampled_ags,sampled_igs], axis=-1)
      ...
      goal_values = self.score_goals(sampled_cgs, AttrDict(q_values=q_values, states=states))

      if self.config.dg_score_multiplier > 1. and self.dg_kde.ready:
        dg_scores = self.dg_kde.evaluate_log_density(sampled_cgs.reshape(self.n_envs * self.num_sampled_ags, -1))
  ...
  def append_initial_goal(self, goal):
    return np.concatenate([goal, self.initial_goals], axis=-1)
...
class HimegaDensityAchievedGoalCuriosity(AchievedGoalCuriosity):
  """
