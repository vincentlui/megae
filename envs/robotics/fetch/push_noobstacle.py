...
MODEL_XML_PATH = os.path.join(dir_path, '../assets/fetch', 'push_noobstacle.xml')

def goal_distance(goal_a, goal_b):
    assert goal_a.shape == goal_b.shape
    return np.linalg.norm(goal_a - goal_b, axis=-1)

class FetchPushNoObstacleEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    ...
    def render(self, mode='human', width=500, height=500):
        return super(FetchPushNoObstacleEnv, self).render(mode, width, height)
