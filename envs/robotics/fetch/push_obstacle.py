...
MODEL_XML_PATH = os.path.join(dir_path, '../assets/fetch', 'push_obstacle.xml')

def goal_distance(goal_a, goal_b):
    assert goal_a.shape == goal_b.shape
    return np.linalg.norm(goal_a - goal_b, axis=-1)

class FetchPushObstacleEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        ...
        self.adapt_dict["obstacles"] = [[1.3 - 0.07, 0.75, 0.6 - 0.1, 0.18, 0.04, 0.1]]
        self.adapt_dict["spaces"] = [30, 30, 10] # [30, 30, 10]
        self.adapt_dict["z_penalty"] = 1

        super(FetchPushObstacleEnv, self).__init__(
    ...
    def render(self, mode='human', width=500, height=500):
        return super(FetchPushObstacleEnv, self).render(mode, width, height)
